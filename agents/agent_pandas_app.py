...
def analysis_plan_node(state: PandasState) -> dict:
    system = SystemMessage(content=(
        ...
        f"SCHEMA (use ONLY these real columns):\n{get_schema_string()}\n\n"

        "Choose exactly ONE intent — the single analysis type that best fits:\n"
        "- aggregate: a total, count, sum, average, min, or max.\n"
        "- comparison: compare a metric across groups, OR explain why a metric differs "
        "across groups. Use when there is a CLEAR target variable. For 'why does X "
        "differ' questions, set metric=X, dimension=the grouping, and list 2-3 candidate "
        "driver columns as dimensions.\n"
        "- correlation: the relationship between two numeric variables.\n"
        "- distribution: the spread/shape of a single variable.\n"
        "- trend: how a metric changes over time or an ordered dimension.\n"
        "- ranking: top/bottom entities by a metric.\n"
        "- exploratory: use ONLY when the question has NO clear target variable and asks "
        "to discover patterns or anomalies openly (e.g. 'find unusual purchasing "
        "patterns'). If the question names a specific thing to explain, measure, or "
        "compare, it is NOT exploratory — pick the intent matching that target. "
        "Phrases like 'explain why', 'what drives', 'what affects' name a target: use "
        "comparison, not exploratory.\n\n"

        "PLAN FIELDS:\n"
        "- metrics: the numeric column(s) to measure. REAL schema columns only. Do NOT "
        "invent derived names like 'total_spend_per_customer' — those are computed later "
        "in the analysis code.\n"
        "- grain: what one row represents (order, order_item, customer).\n"
        "- dimensions: what to break down, compare, or investigate by.\n"
        "- filters: conditions to apply.\n"
        "- comparison: for comparison intent, name the groups or the metric-vs-driver "
        "relationship being explained.\n\n"

        "KEEP IT FOCUSED. Pick at most 3-4 metrics and 2-3 dimensions — the ones MOST "
        "likely to answer THIS specific question. Do NOT list every column that could "
        "conceivably relate; a plan with 8+ metrics or dimensions is too broad. Narrow "
        "to the strongest candidates.\n\n"

        "Plan for THIS question only."
    ))
